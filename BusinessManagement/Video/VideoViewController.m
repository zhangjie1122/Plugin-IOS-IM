...
- (id)initWithCallerName:(NSString *)name andVoipNo:(NSString *)voipNo andCallstatus:(NSInteger)status {
    ...
    allWidth = [UIScreen mainScreen].bounds.size.width;
...
- (void)setDeleteData:(NSString*)commandId delegate:(id)delegate
{
...
- (void)viewDidLoad {
    ...
    pointImg.frame = CGRectMake(0.0f, 0.0f, allWidth, 29.0f);
    ...
    videoIcon.center = CGPointMake(allWidth/2, 213.0f);
    ...
    UIView *tmpView1 = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, allWidth, self.bgView.frame.size.height - 54.0f)];
...
- (void)backFront {
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(asynResultCallback: data: status:)]==YES){
        NSString* time = @"";
        if (hhInt > 0) {
            time = [NSString stringWithFormat:@"%02d:%02d:%02d",hhInt,mmInt,ssInt];
        } else {
            time = [NSString stringWithFormat:@"%02d:%02d",mmInt,ssInt];
        }
        [self.delegate asynResultCallback:self.commandId data:[NSString stringWithFormat:@"\"videoMessage:\":\"%@\",\"videoMessage:\":\"%@\",\"videoMessage:\":\"VIDEO\"",self.voipNo,time] status:YES];
    }
}
...
- (void)hangup {
    ...
    statusLabel.text = @"正在挂机...";
    [NSTimer scheduledTimerWithTimeInterval:0.5f target:self selector:@selector(backFront) userInfo:nil repeats:NO];
...
-(void)makeCallViewLayout {
    ...
    if (self.makeCallView == nil) {
        ...
        hangupBtn.frame = CGRectMake(15.0f, 6.0f, allWidth-29.0f, 42.0f);
...
-(void)incomingCallViewLayout {
    ...
    if (self.incomingCallView == nil) {
        ...
        self.incomingCallView.backgroundColor = [UIColor clearColor];
...
//        self.incomingCallView.backgroundColor = ACTION_CALL_VIEW_BACKGROUNTCOLOR;
        ...
        answerBtn.frame = CGRectMake(15.0f, 6.0f, allWidth-120.f, 42.0f);
        ...
        hangupBtn.frame = CGRectMake(allWidth-120.f+15.0f+13.0f, 6.0f, 76.0f, 42.0f);
...
-(void)callingViewLayout {
    ...
    if (self.callingView == nil) {
        ...
        tempHangupButton.frame = CGRectMake(15.0f, 6.0f, allWidth-29.0f, 42.0f);
        ...
        UILabel *timeLabeltmp = [[UILabel alloc] initWithFrame:CGRectMake(allWidth-120.0f, 15.0f, 55.0f, 24.0f)];
        ...
        if (self.cameraInfoArr.count>1) {
            ...
            switchBtn.frame = CGRectMake(allWidth-90.0f, 35.0f, 70.0f, 35.0f);
